            ...
            initWorldData() {
                ...
                for (let i = 0; i < this.worldCountryData.length; i++) {
                    ...
                    api.getCountryLatest(cname).then(
                        res => {
                            if (res != null) {
                                that.worldCountryData[i].value = res.confirmed;
                                that.worldCountryData[i].cured = res.cured;
                                that.worldCountryData[i].dead = res.dead;
                            }
                            if (this.worldCountryData.length - 1 === i) {
                                that.worldConfigure(that.worldCountryData);
                            }
                        }
                    )
                }
